	...
	: QObject(), _parser(par), _adc(adc)
{
	...
	QObject::connect(par, &parser::disconnected, [=] {
		...
		while (!_requests.empty()) { 
			// тушим те запросы которые остались (вдруг скалярные какие-то есть) и они сами себя удаляют из списка
...
request * session::_router(command cmd)
{
	if (cmd.methodId == "ping") {
		...
		// и могут быть прерваны дисконнектом следует связать сигналом stopAll
